...
export class ConstitutionService {
  ...
  static assess(userId: string, answers: any[]) {
    ...
    for (const [type, score] of Object.entries(constitutionScores)) {
      if (type !== '平和质') nonPingHeScores[type] = score
    }

    const sortedScores = Object.entries(nonPingHeScores).filter(([_, score]) => score > 0).sort((a, b) => b[1] - a[1])

    let resultType, confidence
    if (sortedScores.length === 0) { resultType = '平和质'; confidence = 'high' }
    else if (sortedScores.length === 1) { resultType = sortedScores[0][0]; confidence = sortedScores[0][1] >= 3 ? 'high' : 'medium' }
    else { resultType = sortedScores[0][0]; confidence = sortedScores[0][1] >= 3 ? 'medium' : 'low' }
