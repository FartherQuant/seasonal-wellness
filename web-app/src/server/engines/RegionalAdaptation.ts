...
export class RegionalAdaptation {
  ...
  static calculateSeasonalBias(term: string, cityData: any) {
    ...
    if (hotTerms.includes(term)) {
      if (climate_type.includes('温带大陆性') || climate_type.includes('温带季风')) {
        bias.temp_adjustment = 1.1; bias.diet_adjustment = '注重滋阴润燥'
        bias.note = '北方夏季偏干热，清热同时需防燥'
      } else if (climate_type.includes('亚热带')) {
        bias.humidity_adjustment = 1.2; bias.diet_adjustment = '注重清热祛湿'
        bias.note = '江南夏季湿热并重，需清热祛湿并举'
      } else if (climate_type.includes('南亚热带') || climate_type.includes('热带')) {
        bias.temp_adjustment = 1.3; bias.diet_adjustment = '注重清热解暑'
        bias.note = '岭南夏季酷热，清热解暑为首要'
      }
    } else if (coldTerms.includes(term)) {
      if (climate_type.includes('温带大陆性') || climate_type.includes('温带季风')) {
        bias.temp_adjustment = 1.2; bias.diet_adjustment = '注重温补驱寒'
        bias.note = '北方冬季严寒，温补驱寒为首要'
      } else if (climate_type.includes('亚热带')) {
        bias.humidity_adjustment = 1.1; bias.diet_adjustment = '注重温补兼祛湿'
        bias.note = '江南冬季湿冷，温补需兼顾祛湿'
      } else if (climate_type.includes('南亚热带') || climate_type.includes('热带')) {
        bias.temp_adjustment = 0.9; bias.diet_adjustment = '适度温补'
  ...
  static getGenericAdaptation(_term: string) {
    return {
