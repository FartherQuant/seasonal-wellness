...
router.get('/cities', (req: Request, res: Response) => {
  try {
    ...
    res.json({ cities: cities.map((c: any) => ({ name: c.name, province: c.province, region: c.region })), total: cities.length })
  } catch (err: any) { console.error('读取城市数据失败:', err); res.status(500).json({ error: '读取城市数据失败', message: err.message }) }
})
...
router.get('/:city', async (req: Request, res: Response) => {
  ...
  } catch (err: any) { console.error('获取天气失败:', err); res.status(500).json({ error: '获取天气失败', message: err.message }) }
})
