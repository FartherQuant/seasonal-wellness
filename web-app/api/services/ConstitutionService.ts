...
export class ConstitutionService {
  ...
  static getBaseConstitutionInfo(constitution: string) {
    const infoMap: Record<string, any> = {
      '平和质': {
        name: '平和质',
        description: '阴阳平衡，体质正常，适应能力强',
        features: ['面色红润', '精力充沛', '适应力强', '睡眠好'],
        wellness_principle: '维持平衡，顺应节气养生',
        key_advice: '保持规律作息，均衡饮食，适量运动',
      },
      '气虚质': {
        name: '气虚质',
        description: '气虚为主，疲乏、气短、懒言、动则出汗',
        features: ['疲乏', '气短', '懒言', '动则出汗', '易感冒'],
        wellness_principle: '补气健脾，固表护卫',
        key_advice: '多食黄芪、党参、山药、大枣；避免过劳和生冷',
      },
      '阳虚质': {
        name: '阳虚质',
        description: '阳气不足，怕冷、手足不温、喜热饮',
        features: ['怕冷', '手足不温', '喜热饮', '大便稀溏'],
        wellness_principle: '温阳散寒，温补肾阳',
        key_advice: '多食生姜、桂圆、羊肉、核桃；避免寒凉食物和过度运动',
      },
      '阴虚质': {
        name: '阴虚质',
        description: '阴液不足，口燥咽干、手足心热、易失眠',
        features: ['口燥咽干', '手足心热', '易失眠', '大便干结'],
        wellness_principle: '养阴润燥，滋阴降火',
        key_advice: '多食银耳、百合、梨、石斛；避免辛辣和熬夜',
      },
      '痰湿质': {
        name: '痰湿质',
        description: '痰湿内盛，肥胖、胸闷、痰多、腹胀',
        features: ['肥胖', '胸闷', '痰多', '腹胀', '大便黏腻'],
        wellness_principle: '化痰祛湿，健脾理气',
        key_advice: '多食薏米、赤小豆、冬瓜、陈皮；避免油腻和甜食',
      },
      '湿热质': {
        name: '湿热质',
        description: '湿热内蕴，口苦、易长痘、小便黄、大便黏腻',
        features: ['口苦', '易长痘', '小便黄', '大便黏腻', '易急躁'],
        wellness_principle: '清热利湿，调理脾胃',
        key_advice: '多食绿豆、冬瓜、苦瓜、薏米；避免辛辣和油炸',
      },
      '血瘀质': {
        name: '血瘀质',
        description: '血瘀内阻，面色晦暗、易长斑、痛经、关节痛',
        features: ['面色晦暗', '易长斑', '痛经', '关节痛', '嘴唇暗'],
        wellness_principle: '活血化瘀，疏通气血',
        key_advice: '多食山楂、玫瑰花、红糖；避免生冷和久坐',
      },
      '气郁质': {
        name: '气郁质',
        description: '气机郁滞，情绪低落、胸闷、喜太息、易怒',
        features: ['情绪低落', '胸闷', '喜太息', '易怒', '睡眠差'],
        wellness_principle: '疏肝解郁，调畅情志',
        key_advice: '多食玫瑰花、薄荷、佛手；避免压抑和过度思虑',
      },
      '特禀质': {
        name: '特禀质',
        description: '先天禀赋异常，易过敏、易感冒、易哮喘',
        features: ['易过敏', '易感冒', '易哮喘', '皮肤敏感'],
        wellness_principle: '益气固表，抗过敏，避免接触过敏原',
        key_advice: '多食黄芪、大枣、山药；避免海鲜、芒果、菠萝等易过敏食物',
      },
    }
  ...
  static getQuestionnaire() {
    try {
      ...
      return {
        ...
        constitution_info: data.constitution_info || {},
      }
    } catch (err) {
      console.error(`Error loading constitution_questionnaire.json: ${err.message}`)
  ...
  static assess(userId: string, answers: any[]) {
    ...
    const nonPingHeScores: Record<string, number> = {}
    ...
    const sortedScores = Object.entries(nonPingHeScores)
  ...
  static getUserConstitution(userId: string) { return null }
  static getAssessmentHistory(userId: string) { return [] }
