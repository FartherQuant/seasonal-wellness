...
export class RecipeService {
  static getDailyRecipe(term: string, constitution: string, weather: string | null = null, date: string | null = null, opts: { bypassLRU?: boolean } = {}) {
    ...
    let usedMain: Set<string> = opts?.bypassLRU ? new Set() : (() => {
      const lruKey = `${term}#${constitution}`
      let lruEntry = termLRU.get(lruKey)
      if (!lruEntry || lruEntry.lastTerm !== term) { lruEntry = { used: new Set(), lastTerm: term }; termLRU.set(lruKey, lruEntry) }
      if (!lruEntry?.used) { lruEntry = { used: new Set(), lastTerm: term }; termLRU.set(lruKey, lruEntry) }
    ...
    const getStyle = (name: string) => { for (const kw of STYLE_KEYWORDS) { if (name.includes(kw)) return kw } return null }
    const getWords = (name: string) => { const words: string[] = []; for (let i = 0; i < name.length - 1; i++) { const w = name.slice(i, i + 2); if (/[\u4e00-\u9fff]/.test(w[0]) && /[\u4e00-\u9fff]/.test(w[1])) { words.push(w); } } return words; }
    const usedWords = new Set<string>()
    for (const item of usedMain) for (const w of getWords(item)) usedWords.add(w)

    const isUsed = (main: string | undefined) => {
      if (!main) return false
      if (usedMain.has(main) || todayUsed.has(main)) return true
      for (const ex of [...usedMain, ...todayUsed]) { if (main.includes(ex) && ex.length >= 2) return true; if (ex.includes(main) && main.length >= 2) return true }
      const newWords = getWords(main)
      for (const w of newWords) { if (usedWords.has(w)) return true }
      return false
    }

    const pickFrom = (list: string[], slotOffset: number) => {
      if (!list || list.length === 0) return null
      const candidates = list.filter(r => { const main = this.getMainIngredient(r); if (isUsed(main)) return false; const style = getStyle(r); if (style && todayStyles.has(style)) return false; return true })
    ...
    if (!opts?.bypassLRU) {
      ...
      if (lruEntry && lruEntry.used) { lruEntry.used = usedMain; termLRU.set(lruKey, lruEntry) }
    }
    if (termLRU.size > MAX_LRU_SIZE) { const firstKey = termLRU.keys().next().value; termLRU.delete(firstKey) }
  ...
  static generateSeed(date: string) { const d = new Date(date); return d.getFullYear() * 10000 + (d.getMonth() + 1) * 100 + d.getDate() }
